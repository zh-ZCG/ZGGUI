...
const zTagImageStyle = computed(() => {
  let style = {
    width:
      props.size === 'large' ? '18px' : props.size === 'normal' ? '16px' : '14px',
...
const zTagTextStyle = computed(() => {
  let style = {
    fontSize:
      props.size === 'large' ? '16px' : props.size === 'normal' ? '14px' : '12px',
      ...
      >
        <slot name="icon">
          <div style="margin-right: 4px" v-if="props.icon">
            ...
            ></image>
            <zIcon v-else :name="props.icon" :otherStyle="zTagIconStyle"></zIcon>
          </div>
        </slot>
